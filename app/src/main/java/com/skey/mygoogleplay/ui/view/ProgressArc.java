...
public class ProgressArc extends View {
	...
	private Drawable mDrawableForeground;// 图片
	...
	public void seForegroundResource(int resId) {
		...
		mDrawableForegroundResId = resId;
		mDrawableForeground = UIUtils.getDrawable(mDrawableForegroundResId);
		invalidateSafe();
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (widthMode == MeasureSpec.EXACTLY) {// 如果是精确的
			...
			width = mDrawableForeground == null ? 0 : mDrawableForeground
					.getIntrinsicWidth();
		...
		if (heightMode == MeasureSpec.EXACTLY) {// 如果是精确的
			...
			height = mDrawableForeground == null ? 0 : mDrawableForeground
					.getIntrinsicHeight();
	...
	protected void onDraw(Canvas canvas) {
		if (mDrawableForeground != null) {// 先把图片画出来
			mDrawableForeground.setBounds(0, 0, getMeasuredWidth(),
					getMeasuredHeight());
